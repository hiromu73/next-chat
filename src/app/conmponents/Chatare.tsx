...
const Chatare = () => {
  ...
  const messagShow = message.length > 2;
  const [showDialog, setShowDialog] = useState(messagShow);
  useEffect(() => {
    setShowDialog(message.length > 2);
  }, [message]);

  // openがflaseだとgameOver componentを表示する. →score表示が必要
  return test_1 ? (
    <>
      {showDialog && <FloatingDialog open={open} setOpen={setOpen} />}
      <Box
        sx={{
